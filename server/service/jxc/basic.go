...
func (s *BasicService) UpdateCategory(ctx context.Context, c *jxc.GoodsCategory) error {
	return global.GVA_DB.WithContext(ctx).Omit("code").Save(c).Error // 忽略code字段，避免更新时修改唯一约束
}
...
func (s *BasicService) UpdateBrand(ctx context.Context, b *jxc.Brand) error {
	return global.GVA_DB.WithContext(ctx).Omit("code").Save(b).Error // 忽略code字段，避免更新时修改唯一约束
}
...
func (s *BasicService) UpdateSupplier(ctx context.Context, spli *jxc.Supplier) error {
	return global.GVA_DB.WithContext(ctx).Omit("code").Save(spli).Error // 忽略code字段，避免更新时修改唯一约束
}
...
func (s *BasicService) UpdateWarehouse(ctx context.Context, w *jxc.Warehouse) error {
	return global.GVA_DB.WithContext(ctx).Omit("code").Save(w).Error // 忽略code字段，避免更新时修改唯一约束
}
